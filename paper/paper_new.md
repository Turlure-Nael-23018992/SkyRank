...
To our knowledge, the only open-source visualization-focused Skyline tools are Skylens implementations such as: Zhao et al. student implementation [@zhaoSkyLensVisualAnalysis2018Implementation], `skyline` by Miguel Mota [@motaSkylineDivideConquer2018] and `gh-skyline` by GitHub [@githubGitHubSkyline3D2023]. SkyLens is a visual analytics system designed specifically to explore, compare, and interpret Skyline points across multidimensional datasets [@zhaoSkyLensVisualAnalysis2017].

There are a few open-source tools for queries and Skyline visualization, but none for ranking. Examples include: `gkoos/skyline` [@koosSkylineGoLibrary2016], `SkylineProf` [@yuSkylineInteractiveInEditor2020], and `SkylineDet-YOLOv11Seg` [@kuazhangxiaoaiYUNetImprovedYOLOv112025].
